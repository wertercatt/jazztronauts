    ...
    try:
        ...
        out = check_output(
            [
                ...
                tempdir,
                "-out",
                os.path.join(os.getcwd(), gmaname),
            ],
            encoding="UTF-8",
        )
        print(out)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        dest="changelog",
        type=str,
        help="Text for what to put in the change log",
    )
    parser.add_argument(
        "-b", "--build", dest="build", action="store_true", help="Enable building gmas"
    )
    parser.add_argument(
        "-u",
        "--upload",
        dest="upload",
        action="store_true",
        help="Enable uploading of gmas to workshop",
    )
    ...
    for pack in allpacks:
        print(f"{pack:<35}: {sizeof_fmt(getPackSize(pack, filemap))}")
